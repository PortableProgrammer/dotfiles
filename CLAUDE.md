...
- **Common vs mac separation**: Common package has cross-platform files (shell config, nano, screen); mac package adds macOS-specific files (`.gitconfig` for identity/signing, `.ssh/config` for 1Password agent, `.zshrc.d/` mac scripts). Claude Code's `~/.claude/` config is managed by the private claude-ops repo, not here.
